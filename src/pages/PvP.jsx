...
export default function PvP(){
 ...
 useEffect(()=>{
  ...
  const ch=supabase.channel(`rockey-pvp:${room.id}`,{config:{broadcast:{ack:true}}})
   ...
   .on("postgres_changes",{event:"UPDATE",schema:"public",table:"pvp_rooms",filter:`id=eq.${room.id}`},(payload)=>{const next=payload?.new;if(next?.guest_id&&!opponent){setOpponent({name:next.guest_login_id,rating:Number(next.guest_rating||0)});setStatus("상대가 연결되었습니다. 대국을 시작하세요!")}});
