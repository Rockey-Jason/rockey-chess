...
export default function Shop(){
 ...
 const load=async()=>{const {data:{user}}=await supabase.auth.getUser();setUser(user);if(!user)return;const {data:u}=await supabase.from("users").select("doldolcoin").eq("id",user.id).maybeSingle();setCoins(Number(u?.doldolcoin||0));const {data:i}=await supabase.from("user_inventory").select("item_id").eq("user_id",user.id);setOwned((i||[]).map(x=>x.item_id));};
 useEffect(()=>{load()},[]);
